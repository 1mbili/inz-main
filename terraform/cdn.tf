...
resource "azurerm_network_interface" "cdn_nic" {
  name                = "cdn-0-NIC"
  location            = azurerm_resource_group.example.location
  resource_group_name = azurerm_resource_group.example.name
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.cdn_public_ip.id
  }
}
...
resource "azurerm_linux_virtual_machine" "cdn" {
  ...
  location              = azurerm_resource_group.example.location
  resource_group_name   = azurerm_resource_group.example.name
  network_interface_ids = [azurerm_network_interface.cdn_nic.id]
  size                  = "Standard_B1s"
